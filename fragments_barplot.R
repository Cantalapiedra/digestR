...
counts <- table(df_bed$chr, df_bed$fragtype)
...
colnames(counts) <- c(orig_colnames, "total") # recover column names
...
colnames(counts) <- rownames(t_counts) # recover column names
